...
function articleHoverHandler(info, i) {
  ...
  if (i != 0)
    articles[i - 1].querySelector(".info").classList.remove("border-b");
...
articles.forEach((article, i) => {
  ...
  article.addEventListener(
    "mouseover",
    articleHoverHandler.bind(null, info, i),
  );
});
...
btnSlides[0].addEventListener("click", () => {
  galaxyAiHandler(0);
});
btnSlides[1].addEventListener("click", () => {
  galaxyAiHandler(1);
});
